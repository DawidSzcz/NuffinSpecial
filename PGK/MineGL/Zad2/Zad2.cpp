...
{	
	...
	GLfloat cosin = resize(argc-3, argv[1]);
	...
	GLfloat vertexes[] = {
    		-cosin, 1.0f, 0.0f, 1.0f,
     		cosin, 1.0f, 1.0f, 1.0f,
     		-cosin,  -1.0f, 0.0f, 0.0f,
			cosin, -1.0f, 1.0f, 0.0f
	};  
	...
	*/
		if(!is3d())
		{
			...
			glBindBuffer(GL_ARRAY_BUFFER,VBAPATH[0][1]);
			glBufferData(GL_ARRAY_BUFFER, data[0].size()* sizeof(GLfloat), &data[0].front(), GL_STATIC_DRAW);
			glVertexAttribPointer(
				            2,                  // attribute. No particular reason for 0, but must match the layout in the shader.
				            3,                  // size
				            GL_FLOAT,           // type
			...
			glEnableVertexAttribArray(2);	
			glDrawArrays(GL_LINE_STRIP, 0, data[0].size()/6);
		}
		else
		{
			glUseProgram(programID2);	
			glUniformMatrix4fv(vec2, 1, GL_FALSE, glm::value_ptr(cameraPos));
			glBindBuffer(GL_ARRAY_BUFFER,VBAPATH[0][0]);
			glBufferData(GL_ARRAY_BUFFER, data[0].size()* sizeof(GLfloat), &data[0].front(), GL_STATIC_DRAW);
			glVertexAttribPointer(
				           2,                  // attribute. No particular reason for 0, but must match the layout in the shader.
				            3,                  // size
			...
			glEnableVertexAttribArray(2);	
			glDrawArrays(GL_TRIANGLE_STRIP, 0, data[0].size()/3);		
		}
